...
| # | 严重度 | 一句话 | 复现 | 状态 |
|---|---|---|---|---|
| [A1](#a1) | 1 静默 | 429 / 5xx / 401 / 402 被记成「模型不支持工具」并持久化 | 已复现 | 未修 |
| [A2](#a2) | 1 静默 | 火山方舟 2.1 的 `encrypted_content` 不留存、不回传 | — | 未修 |
| [A3](#a3) | 1 静默 | ② 关思考不发任何字段，照想照计费 | — | 未修 |
| [A4](#a4) | 1 静默 | ② 无 system 消息时不发 `instructions`，中转站注入数千 token | 已复现 | 未修 |
| [A5](#a5) | 1 静默 | ① 同一 delta 两个推理字段都非空时拼接两遍 | 已复现 | 未修 |
| [A6](#a6) | 1 静默 | ④ 没有「关思考」字段，默认开思考的镜像关不掉 | — | 未修 |
| [A7](#a7) | 1 静默 | 生成请求遇 504 / 408 会重发，可能重复计费 | — | 未修 |
| [A8](#a8) | 2 会响 | 智谱 glm-5.3 每个请求都 400，且不学习 | 已复现 | 未修 |
| [A9](#a9) | 2 会响 | 400 学习只读 message，不读 `error.param` | 已复现（文案待核实） | 未修 |
| [A10](#a10) | 2 会响 | 粘贴完整端点地址后路径拼错；错误信息不带 URL | 已复现 | 未修 |
| [A11](#a11) | 2 会响 | 缺终止事件、坏 SSE 事件都让整条请求失败 | — | 待决定 |
| [A12](#a12) | 2 会响 | 背靠背拼接的多个 JSON 参数被判非法 | — | 未修 |
| [A13](#a13) | 4 结构 | ④ 思考固定 `enabled + budget_tokens`、不带 `display`；被拒后静默关思考 | — | 未修 |

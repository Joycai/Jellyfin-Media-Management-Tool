...
- **`AppBackdrop` ([app_backdrop.dart](lib/widgets/ui/app_backdrop.dart)) bakes the window gradients into an image** (1/4 resolution, window aspect, quantised size, 120ms debounce, disposed on replace, blur with `TileMode.clamp`). Never draw a second full-window gradient live — that alone cost ~21 ms. `RepaintBoundary` cannot substitute.

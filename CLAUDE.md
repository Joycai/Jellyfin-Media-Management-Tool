...
- **The bake's downscale is counted in *device* pixels** — 2 per texel for the sharp image, 4 for the blurred copies. A gradient is rasterized *with dither*, and the stretch magnifies that noise: sizing off logical pixels made it 8 device px per texel on a Retina display and the backdrop read as a dishcloth. Blurring the dither away is not the fix — it trades the grain for contour banding.

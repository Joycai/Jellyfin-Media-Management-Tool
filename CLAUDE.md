...
- Dart SDK `^3.10.4`. Current app version: `0.20.0+16`.
...
- **A round that leaves the task done ends the run** (unless `stopWhenDone: false`, which direct extraction uses so the model may keep adding fields), so a small model does not spend a turn announcing it has finished. A model that stops early is nudged at most twice.

...
4. **Analyze** — `AiService.analyzeFolder` walks the tree (**capped at 400 files**, dotfiles skipped), builds prompts via `AiPrompt`, and calls the active provider. When the profile names a context window or an output cap, `AiPrompt.batchEntries` splits the files so every request fits — each file is charged for its prompt line *and* the action it comes back as, which is the larger half — later batches are told the title folders earlier ones chose, and `OrganizePlan.merge` joins the results. With neither set it is one request, as it always was.
5. **Provider** — `OpenAiProvider` (`/chat/completions`, JSON mode) or `GoogleGenAiProvider` (`:generateContent`), both through `AiHttp.withRetry` on the cancel token's client. `OpenAiProvider` **streams** and times out on *silence*, not duration — 10 min for the first event (model load + prompt processing), 2 min between events — and never retries a timeout. A 27B model on LM Studio took 88 s for thirty files, so the old 120 s total failed any real folder, and each retry restarted a generation the server was still running; closing the stream is also what makes a local server stop generating on cancel. A body that is not an event stream is parsed as ordinary JSON, and `stream_options` is dropped if refused. `GoogleGenAiProvider` keeps a 120 s total timeout. JSON mode steps down `json_object` → a permissive `json_schema` → none when a 400 names `response_format` (LM Studio accepts only `json_schema`), and the accepted mode is cached per endpoint + model. An OpenAI-compatible profile needs no API key (`AiProviderTypeX.requiresApiKey`); a blank key sends no `Authorization` header at all.
6. **Parse** — `OrganizePlan.fromAiJson` produces `OrganizeAction`s. Confidence `< 0.6` → `ActionStatus.needsReview`. A batch cut off at its output limit (`AiResponse.truncated`) fails with a message naming the budget rather than a bare JSON error.
...
- **`AiConfig.contextWindow` is a budget, not a server setting.** Compatible servers fix the context when the model is loaded and accept nothing per request, and local ones do not reject an oversized prompt — they silently drop its front, system prompt first. The app can only send less: organize batches, and `RecipeLearner` / `DirectExtractor` trim their page payload through `TokenBudget`, which over-estimates on purpose (three ASCII characters per token, one token per other character). Null means send everything, as before.

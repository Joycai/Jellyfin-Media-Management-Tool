...
**The user's font choice reaches text only by inheritance, so two things must hold.** `ThemeData.fontFamily` carries the family the user picked (`FontService.familyFor`) and `fontFamilyFallback` carries the platform's CJK families, and every `Text` merges those in from the ambient `DefaultTextStyle`. So: never write a bare `DefaultTextStyle(style: ...)` — it *replaces* the ambient style and silently drops both. Use `DefaultTextStyle.merge`. The status bar, the table footer and `AppColumnHeader` each lost the family that way, and the loss is invisible on a machine where no font has been chosen.

And a style that names its own `fontFamily` must bring `AppTypeScale.monoFallback` with it. `AppTypeScale.mono` is the design's JetBrains Mono, which is neither bundled nor installed on any platform by default, and the pre-redesign code asked for `'monospace'` — a CSS/Android generic that Windows and macOS do not have. With no chain behind them, both fell through to the theme's *CJK* fallback, so every path, version and token count was drawn in Microsoft YaHei: proportional, and not the user's font either. `monoFallback` is one flat list covering all three platforms (unmatched names are skipped) and it ends with the CJK families, because it replaces the theme's and paths contain Chinese directory names.
